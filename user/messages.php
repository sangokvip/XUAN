...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mark_read'])) {
    ...
    if ($messageId) {
        $messageManager->markAsRead($messageId, $userId, 'user');
    }
}

// 处理批量标记为已读
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mark_all_read'])) {
    $allMessages = $messageManager->getUserMessages($userId, 'user', 1000, 0);
    $messageIds = array_column($allMessages, 'id');
    $messageManager->markMultipleAsRead($messageIds, $userId, 'user');
}
...
// 获取消息列表
$messages = $messageManager->getUserMessages($userId, 'user', $limit, $offset);

// 自动标记所有未读消息为已读
$unreadMessages = array_filter($messages, function($msg) { return !$msg['is_read']; });
if (!empty($unreadMessages)) {
    $unreadMessageIds = array_column($unreadMessages, 'id');
    $messageManager->markMultipleAsRead($unreadMessageIds, $userId, 'user');
...
// 获取未读消息数量
$unreadCount = $messageManager->getUnreadCount($userId, 'user');
...
<body>
    ...
    <div class="messages-container">
        ...
        <div class="messages-actions">
            ...
            <div>
                <a href="index.php" class="btn btn-secondary">← 返回用户中心</a>
                <?php if ($unreadCount > 0): ?>
